...
func (u user) FindByQuery(searchQuery string) ([]models.User, error) {
	...
	err := u.db.Select(&queryUsers, fmt.Sprintf("select * from users where lower(firstname) like lower('%s') or lower(secondname) like lower('%s') or lower(nick) like lower('%s') LIMIT 5", searchFormat, searchFormat, searchFormat))
...
func (u user) FindByLogin(login string) (models.User, error) {
	...
	err := u.db.Get(&resultUser, fmt.Sprintf("select * from users where nick = '%s'", login))
...
func (u user) UserExists(login string) (models.User, error) {
	...
	err := u.db.Get(&resultUser, fmt.Sprintf("select * from users where nick = '%s' or email = '%s'", login, login))
